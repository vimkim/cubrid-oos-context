...
> Last updated: 2026-07-02 | Branch: `feat/oos` | Milestone: M2 (the only active milestone — all remaining OOS work; M3 & M4 cancelled)
...
| Concept | Detail |
|---------|--------|
| OOS trigger | record > `DB_PAGESIZE/4` (4KB on 16KB pages) → demote largest variable columns (value > `OR_OOS_INLINE_SIZE` = 16B) one-by-one until record fits (PG TOAST-style; CBRD-26776). Type-agnostic — BLOB/CLOB locators demote like any variable value (ADR-0002) |
...
2. **Eligibility**: a column is a candidate iff `is_variable && column_size > OR_OOS_INLINE_SIZE` (16B) — i.e. its value is bigger than the 16-byte OOS stub (OID + length) that replaces it, so demoting it actually shrinks the record. The rule is **type-agnostic**: BLOB/CLOB columns are eligible too (ADR-0002) — the in-row value is the ELO locator string, and only those locator bytes go to OOS (the LOB payload stays in external LOB storage). LOB copy semantics are preserved on the OOS path: `heap_attrinfo_dbvalue_to_recdes` performs the same `db_elo_copy_with_prefix` step as the inline writer before serializing.
...
| OOS stub writer bounds-checked the wrong pointer | `heap_attrinfo_transform_variable_to_disk` checked `buf->ptr` (still at the VOT, record front) instead of the real stub write position `*ptr_varvals` (record tail), then wrote 16B there — bypassing the `S_DOESNT_FIT` → grow-and-retry safety net. Crashed `bug_xdbms3693` (100 LOB columns; locator regeneration mid-write made the size estimate drift). Debug: `or_put_oid` assert core; release: silent 8–16B heap overrun | Server crash / memory corruption on records whose actual writes exceed the sizing estimate. _Fixed on `oos-m2-all-plans-experimental` 2026-07-02 (check `*ptr_varvals` — the bounds fix alone; LOB columns stay demotable per ADR-0002), pending merge. Detail: `feat-oos-m2-manual` `docs/manual-failure-fix-plans/03-appendix-ptr-varvals-write-position.md`_ | — |
